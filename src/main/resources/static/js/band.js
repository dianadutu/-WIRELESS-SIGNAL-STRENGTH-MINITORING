document.addEventListener('DOMContentLoaded', function() {
    ...
    // Verifică fiecare rețea individuală
    networksData.forEach((network, index) => {
        console.log(`Network ${index}:`, network.bssids.length);
    });
    ...
    for (let i = 0; i < networksData.length; i++) {
        for (let j = 0; j < networksData[i].bssids.length; j++) {
            if(networksData[i].bssids[j].band === "2.4 GHz") {
                let data = {
                    channel : networksData[i].bssids[j].channel,
                    signal : networksData[i].bssids[j].signal,
                    ssid : networksData[i].ssid
                }
                signal2.push(data);
            }
            if(networksData[i].bssids[j].band === "5 GHz") {
                let data = {
                    channel : networksData[i].bssids[j].channel,
                    signal : networksData[i].bssids[j].signal,
                    ssid : networksData[i].ssid
